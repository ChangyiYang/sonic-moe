...
class Token_gather_and_sum_over_k:
    def __init__(
        ...
        handled_rows_per_block, thread_per_row, handled_rows_size = self._calculate_launch_settings()

        self.smem_size = (handled_rows_per_block * handled_rows_size * K_STAGED) * x_dtype.width // 8 + (
            handled_rows_per_block * MAX_K
    ...
    def _make_tiled_copy_2D(
        ...
        thread_layout = cute.make_layout(
            (self.num_threads // self.thread_per_row, self.thread_per_row), stride=(self.thread_per_row, 1)
        )
    ...
    def _make_tiled_copy_3D(
        ...
        thread_layout = cute.make_layout(
            (self.num_threads // self.thread_per_row, self.thread_per_row, 1), stride=(self.thread_per_row, 1, 0)
        )

        # Value layout for copy
        value_layout = cute.make_layout((1, self.copy_elems, self.K_STAGED), stride=(0, 1, self.copy_elems))
    ...
    def _calculate_launch_settings(self):
        ...
        for t in range(get_next_power_of_2(thread_per_row), 0, -8):
            if (
                2 * self.copy_elems * t + 2 * t * K_STAGED <= min(t * self.REG_LIMIT, 65536)
    ...
    def __call__(
        ...
        X_g2s_copy_atom = cute.make_copy_atom(
            cute.nvgpu.cpasync.CopyG2SOp(cache_mode=cute.nvgpu.cpasync.LoadCacheMode.NONE),
            mX.element_type,
        ...
        self.kernel(mX, mW, mO, mMoffset, mXIdx, X_tiled_copy, O_tiled_copy, tile_sched_params, TileScheduler).launch(
            grid=grid,
    ...
    @dsl_user_op
    def fma(self, a: float | Float32, b: float | Float32, c: float | Float32, *, loc=None, ip=None) -> Float32:
        return Float32(
            llvm.inline_asm(
    ...
    def kernel(
        ...
        sX_slice = smem.allocate_tensor(
            mX.element_type,
            cute.make_ordered_layout((handled_rows_per_block, handled_rows_size, K_STAGED), order=(1, 0, 2)),
            byte_alignment=16,
        )
        sW = None
        ...
        while work_tile.is_valid_tile:
            ...
            cO = cute.local_tile(idO, (handled_rows_per_block, handled_rows_size), (M_idx, 0))
            ...
            if tOcO[0, 0][0] < MAX_M:
                ...
                for i in cutlass.range(cute.ceil_div(K, thread_per_row), unroll_full=True):
                    ...
                    if fetched_idx < K:
                        ...
                        if const_expr(mW is not None):  # otherwise, we do sequential scan
                            sW[row_id_within_block, fetched_idx] = mW[tMIdx_s + fetched_idx]
                ...
                for N_idx in cutlass.range_constexpr(cute.ceil_div(self.N, handled_rows_size)):
                    gO = cute.local_tile(mO, (handled_rows_per_block, handled_rows_size), (M_idx, N_idx))
                        ...
                        for k in cutlass.range(cute.ceil_div(K, K_STAGED), unroll=1):
                            if const_expr(mW is not None):
                                sW_slice = cute.make_tensor(
                                    self.elem_pointer(sW, (row_id_within_block, k * K_STAGED)).align(K_STAGED),
                                    (K_STAGED,),
                                )
                            sXIdx_slice = cute.make_tensor(
                                self.elem_pointer(sXIdx, (row_id_within_block, k * K_STAGED)).align(K_STAGED),
                                (K_STAGED,),
                            )
                            ...
                            for i in cutlass.range_constexpr(K_STAGED):
                                ...
                                if k_idx < K:
                                    ...
                                    XsPtr = self.elem_pointer(
                                        sX_slice, (row_id_within_block, col_id_within_row, i)
                                    ).align(self.universal_copy_bits // self.copy_elems)

                                    Xg_thr_slice = cute.make_tensor(XgPtr, (self.copy_elems,))
                                    Xs_thr_slice = cute.make_tensor(XsPtr, (self.copy_elems,))
                            ...
                            for i in cutlass.range_constexpr(K_STAGED):
                                ...
                                if k_idx < K:
                                    ...
                                    for j in cutlass.range_constexpr(self.copy_elems):
                                        if const_expr(mW is not None):
                                            y[(j, 0), 0, 0] = self.fma(
                                                rW[i].to(cutlass.Float32), rX[j].to(cutlass.Float32), y[(j, 0), 0, 0]
                                            )
                                        else:
                                            y[(j, 0), 0, 0] = rX[j].to(cutlass.Float32) + y[(j, 0), 0, 0]
